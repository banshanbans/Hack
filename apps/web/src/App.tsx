...
function RiskPage() {
  ...
  const navigate = useNavigate();
  const {session, assessment, health} = useApp();
  const onboarding = useOnboarding();
  const assessmentState = useAssessment();
  ...
  return <section className="risk-page">
    <div className="risk-toolbar"><span className="glass-chip"><Icon name="cloud_done" filled />{health === 'demo' ? '演示固定样例 · ' : ''}AI 已识别 {result.risks.length} 处风险</span><button className="glass-button" onClick={() => setZoom(value => value >= 1.8 ? 1 : value + 0.2)} aria-label="放大照片"><Icon name={zoom > 1 ? 'zoom_out_map' : 'zoom_in'} /></button></div>
