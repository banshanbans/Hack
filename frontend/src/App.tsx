...
import {CAMERA_COPY, DIFFICULTY_COPY, HOME_HERO_COPY, PRODUCT_NAME, ROOM_COPY, ROOM_PHOTO_GUIDES, SCENE_ELEMENT_COPY, SEVERITY_COPY, STAGE_COPY} from './content';
...
function PersistentTabBar({pathname}: {pathname: string}) {
  ...
  return <><nav className="persistent-tab-bar three-tabs" aria-label="主导航">
    ...
    {capabilities?.h5_camera !== false ? <button className={`camera-tab ${cameraActive ? 'active' : ''}`} aria-current={cameraActive ? 'page' : undefined} onClick={() => cameraActive ? undefined : setCameraIntroOpen(true)}><span className="camera-tab-icon"><img src="/assets/camera-tab.svg" alt="" /></span><span>相机</span></button> : <button disabled aria-label="相机暂未开放"><span className="camera-tab-icon"><img src="/assets/camera-tab.svg" alt="" /></span><span>相机</span></button>}
...
function CameraPage() {
  ...
  const startCamera = async () => {
    ...
    try {
      if (!session) {
        ...
        setSession({assessment_id: value.assessment_id, access_token: value.access_token, last_route: 'camera'});
      }
      ...
      stopCamera();
      showToast((error as Error).name === 'NotAllowedError' ? '没有获得相机权限，请在浏览器设置中允许或改用照片' : '相机无法开启，请改用照片或视频');
    }
  };
  useEffect(() => {
    ...
    const inspect = async () => {
      ...
      if (local.brightness < 28 || local.brightness > 232 || local.sharpness < 5) { setStatus('请放慢移动，并让通道和地面保持清晰'); return; }
  ...
  const saveEvidence = async () => {
    ...
    try {
      ...
      let room = assessment.rooms.find(item => item.room_type === roomType && item.status !== 'completed');
      if (!room) room = await api.createRoom(roomType);
      await api.uploadMedia(room.room_id, frame.blob, frame.width, frame.height, {sourceKind: 'h5_camera_frame', sourceId: frame.sourceId, capturedAtMs: Date.now(), orientation: 'up', perceptualHash: frame.hash});
      stopCamera();
      navigate(`/upload/${room.room_id}`);
      showToast('已保存为正式检查照片；完成正式分析后才会进入评分');
...
function HomePage() {
  ...
  return <section className="page home-page">
    ...
    <div className="hero-image">
      ...
      <span className="image-callout amber"><Icon name="warning" filled />{HOME_HERO_COPY.rugCallout}</span>
...
function RoomsPage() {
  ...
  if (showTasks) return <section className="page rooms-page">
    ...
    <div className="room-task-list">{taskRooms.map(room => {
    ...
    <button className="button quiet full" onClick={() => { setEditingPlan(true); setMultiMode(true); setSelectedRooms(new Set(savedPlan)); }}>修改检查房间</button>
  ...
  return <section className="page rooms-page">
    ...
    <button className="button primary full" disabled={busy || (multiMode ? !selectedRooms.size : !singleRoom)} onClick={() => saveSelectedRooms(multiMode ? [...selectedRooms] : singleRoom ? [singleRoom] : [])}>{multiMode ? `保存计划（${selectedRooms.size} 个房间）` : singleRoom ? `开始检查${ROOM_COPY[singleRoom].name}` : '请先选择一个房间'}</button>
  </section>;
}
...
function UploadPage() {
  ...
  const videoPreviews = useMemo(() => videoFrames.map(frame => ({id: frame.id, url: URL.createObjectURL(frame.blob)})), [videoFrames]);
  useEffect(() => () => videoPreviews.forEach(item => URL.revokeObjectURL(item.url)), [videoPreviews]);
  if (!session) return <Navigate to="/home" replace />;
  if (loading && !assessment) return <Loading />;
  if (error) return <ErrorState error={error} retry={reload} />;
  ...
  const decodeVideo = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setVideoProgress(0);
    try {
      const frames = await extractVideoFrames(file, setVideoProgress);
      setVideoFrames(frames.slice(0, Math.max(0, 6 - room.media.length)));
      showToast(`已在本地选出 ${Math.min(frames.length, 6 - room.media.length)} 张代表画面，原视频没有上传`);
    } catch (value) { showToast(friendlyError(value)); }
    finally { setBusy(false); setVideoProgress(null); }
  };
  const uploadVideoFrames = async () => {
    if (!videoFrames.length) return;
    setBusy(true);
    const sourceId = globalThis.crypto?.randomUUID?.() || `video-${Date.now()}`;
    let uploaded = 0;
    for (const frame of videoFrames) {
      try {
        await api.uploadMedia(roomId, frame.blob, frame.width, frame.height, {
          sourceKind: 'video_frame', sourceId, frameIndex: frame.frameIndex,
          capturedAtMs: frame.capturedAtMs, orientation: 'up', perceptualHash: frame.perceptualHash,
        });
        uploaded += 1;
      } catch (value) { showToast(friendlyError(value)); break; }
    }
    setVideoFrames([]);
    setBusy(false);
    reload();
    if (uploaded) showToast(`已上传 ${uploaded} 张代表画面，正在使用现有多图链路检查`);
  };
  const analyze = async () => {
    setBusy(true);
  ...
  const videoMode = assessment?.input_mode === 'video_frame';
  return <section className="page upload-page">
    <div className="page-intro"><h1>{videoMode ? `选择${roomName}视频` : `上传${roomName}照片`}</h1><p>{videoMode ? '视频只在浏览器本地解码，服务端只接收你确认的代表画面。' : '拍摄越完整，分析结果越准确。'}</p></div>
    {videoMode && <>
      <label className="upload-drop video-upload"><input type="file" accept="video/*" onChange={decodeVideo} disabled={busy || room.media.length >= 6} /><span className="upload-icon"><Icon name="video_file" filled /></span><b>{videoProgress !== null ? `正在本地筛选 ${Math.round(videoProgress * 100)}%` : '选择本地视频'}</b><small>不会上传完整视频 · 自动选取 3—6 张不同视角</small></label>
      {videoFrames.length > 0 && <section className="video-frame-review" aria-label="代表画面预览"><div className="section-heading"><div><h2>确认代表画面</h2><p>可删除重复或无关画面，再上传用于正式分析。</p></div><span>{videoFrames.length} 张</span></div><div className="video-frame-grid">{videoFrames.map((frame, index) => <article key={frame.id}><img src={videoPreviews.find(item => item.id === frame.id)?.url} alt={`视频代表画面 ${index + 1}`} /><small>{(frame.capturedAtMs / 1000).toFixed(1)} 秒</small><button className="icon-button" aria-label={`删除代表画面 ${index + 1}`} onClick={() => setVideoFrames(items => items.filter(item => item.id !== frame.id))}><Icon name="close" /></button></article>)}</div><button className="button primary full" disabled={busy} onClick={uploadVideoFrames}>确认并上传代表画面</button></section>}
      <div className="mode-divider"><span>或使用截图 / 照片补充</span></div>
    </>}
    <label className={`upload-drop ${room.media.length ? 'has-media' : ''}`}><input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={upload} disabled={busy || room.media.length >= 6} /><span className="upload-icon"><Icon name={room.media.length ? 'check_circle' : 'photo_camera'} filled /></span><b>{busy ? '正在上传并检查照片…' : room.media.length ? '上传成功' : '拍照或从相册选择'}</b><small>{room.media.length >= 6 ? '已达到 6 张上限' : room.media.length ? `已上传 ${room.media.length} 张 · 点击继续上传` : '最多 6 张，推荐 1—3 张'}</small></label>
...
function ResultPage() {
  ...
  return <section className="page result-page">
    ...
    {activeEvidence && <section className="risk-overview-section"><h2>主要风险</h2><ResultRiskOverlay media={activeEvidence.media} risks={activeEvidence.risks} numberById={numberById} onSelect={riskId => navigate(`/risk/${roomId}/${riskId}`)} />
      ...
      <div className="risk-overview-list">{activeEvidence.risks.map(risk => <button key={risk.risk_id} onClick={() => navigate(`/risk/${roomId}/${risk.risk_id}`)}><span>{numberById[risk.risk_id]}</span><div><b>{risk.title}</b><small>{SEVERITY_COPY[risk.severity]}</small></div><Icon name="chevron_right" /></button>)}</div>
    </section>}
    {!result.risks.length && <p className="empty-copy">当前已检查区域暂未发现明确风险。</p>}
    <div className="result-fixed-actions"><button className="button secondary" disabled={!firstRisk} onClick={() => firstRisk && navigate(`/risk/${roomId}/${firstRisk.risk_id}`)}>查看问题</button><button className="button primary" onClick={() => navigate('/report')}>查看改造清单</button></div>
...
async function createReportImage(report: AssessmentReport): Promise<Blob> {
  ...
  const contentHeight = 760 + risks.length * 190 + risks.reduce((sum, risk) => sum + visibleSolutions(risk.risk_id).reduce((solutionSum, solution) => solutionSum + 250 + solution.actions.length * 45, 100), 0);
  const canvas = document.createElement('canvas');
  canvas.width = 1242;
