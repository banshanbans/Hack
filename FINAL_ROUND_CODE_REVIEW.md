...
项目已经不是概念 Demo：照片 P01—P09、确定性评分、覆盖度、A/B/C 规则价、长图报告、H5 相机、视频抽帧代码、FastAPI/SQLite、iOS RoomPlan/ARKit 均有真实实现，自动测试与 iOS 无签名构建全部通过，公网 H5 与当前构建一致。当前最适合总决赛的是“照片上传 → 风险框 → 可解释评分 → B 档方案 → 预算报告”链路。

但离“现场稳”仍有明显缺口：视频功能没有用户可达入口；结果接口失败会永久显示加载；分析任务缺少幂等与兜底异常收口，服务重启后房间状态还可能残留为“分析中”；无定位证据的候选仍可能进入正式计分；多图/连续帧无法可靠按物理风险去重；上传质量检查与正式分析共用单 Pro 通道，且一次临时 Provider 失败会把正常照片永久判为不可用；iOS 当前不是文档目标中的 Turbo + Pro 正式复核，且没有 LiDAR 真机连续验证。结论是：**不需要大重构，但必须完成一轮小范围、核心链路优先的可靠性收口。**
...
|---|---|---|---|---|
| P01 首页 | 价值主张、示例风险图；点击照片或“视频画面” | 选择入口 | 照片入口创建 `photo` assessment 后进 P02；另一入口实际打开实时相机弹窗 | 照片已完成；视频为高风险断点 |
...
| P04 上传与质量 | 拍照/相册、拍摄建议、缩略图、质量状态 | 1—6 张 JPEG/PNG/WebP；视频模式理论上接视频 | 浏览器转 JPEG/缩放，逐张上传并同步质量检查；有 usable media 才能进 P05 | 照片已完成；视频 UI 不可达；HEIC/多图时延待验 |
...
| 文件处理 | 6/10 | 图片转 JPEG/缩放、视频本地抽帧、Blob URL 清理和服务端体积/签名校验较扎实；HEIC/旋转/大图/视频内存仍待真机，多图逐张质量调用太慢 |
...
#### P0-01 — H5 视频功能代码存在，但普通用户无法进入

- **位置**：`frontend/src/App.tsx:406-411, 652-659`；`frontend/src/content.ts:26`。
- **触发条件**：用户点击首页“从视频画面开始检查”。
- **用户表现**：进入的是实时相机入口，而不是 PRD 要求的 `video_frame` 档案/视频选择；`UploadPage` 的视频 UI 永远收不到 `input_mode=video_frame`。
- **根因**：所有首页创建路径都调用 `createAssessment('photo')`，没有创建 video assessment 的 CTA。
- **修复建议**：把两个入口拆清楚：“上传照片”“选择本地视频”；后者先 `createAssessment('video_frame')` 再进入 P02。实时相机保留为底部中央入口并改正文案。
- **影响**：PRD 明确能力不可验收，评委点击即暴露。
...
- **触发条件**：评委或队友按文档追问“视频入口、Pro 复核、是否已部署”。
- **用户表现**：文档说视频入口已改为实时相机、iOS 日志不再称 Pro，实际源码相反；部分段落同时写“已部署”和“工作区未部署”。
...
| 视频关键帧 | 浏览器亮度/清晰度/感知哈希 | 否 | `FrameSelectionSkill` 可作为命名契约 | 3—6 帧/阈值 | 控制上传阶段 | 是 | 逻辑保留本地，先接通入口和真机方向 |
...
能。首页标题、示例图、风险标注和“上传家中照片”形成明确价值闭环，且免责声明克制。扣分点是“从视频画面开始检查”实际打开实时相机介绍，首次点击会产生认知落差。
