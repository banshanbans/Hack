...
- CocoaPods 仅引入 `VolcEngineRTC/Core` `3.60.15.8550`，版本和校验值由 `apps/ios/Podfile.lock` 锁定。
...
- `httpx==0.28.1` 作为后端运行时依赖，用于服务端带超时、签名和错误映射地调用 `StartVoiceChat` / `StopVoiceChat`；版本同时锁定在 `services/backend/requirements.lock`。
