...
| 比赛 Hero Demo | 已实现 | `demos/hero/` 独立 Three.js 展示前端，已接入经维护者确认授权的 iPhone GLB，并支持原生屏幕网格 VideoTexture、程序化设备兜底、28 秒摄影机时间轴和动态流程兜底；仅用于比赛展示，不代表新增识别能力 |
| 高速 Motion Demo | 已实现首版 | `demos/motion/` 独立 Remotion 工程，包含 BPM 驱动时间轴、可复用 kinetic typography/UI/产品/数据/Logo 镜头、音效点位与约 12.8 秒 1920×1080 验证片；当前未加入第三方音乐，仅用于比赛展示 |
| H5 新手引导与主导航 | 已实现代码，待外部验证 | v2 保存阶段 acknowledged/skipped/completed 结果并迁移 v1；首页集中 AR、照片和通用 AI 入口；普通页固定“首页｜改造方案｜我的”三栏导航 |
| 本机改造历史中心 | 已实现代码 | 本机索引多次 assessment 凭据并迁移原单记录；按检查聚合风险证据、已选方案、预算和效果图，支持逐条服务端删除，不提供账号或跨设备同步 |
| AI 改造效果图 | 已实现代码，待真实模型复验 | 房间卡片进入“选择原图→异步生成→前后对比→细节定位→保存报告”；方案变化后旧图失效，只有用户主动生成且有效保存的版本进入分享报告 |
...
| 首页通用 AI 适老顾问 | 已实现代码，默认关闭 | `/advisor` 独立匿名对话、24 小时滑动恢复、受控知识注入、严格结构化回答、文字删除和纯音频 RTC；不读取档案、照片、房间结果，不创建虚假检查 |
...
- 在真实 Ark/OpenAI 配置下完成一次通用文字问答，并抽样核对受控知识准确性与越界拒答。
- 在 Safari、Chrome 和 App 内 `WKWebView` 完成通用顾问麦克风授权、排队、打断、90 秒停止、切后台释放和原始音频不落盘验证。
- 在 Safari、Chrome 和 App 内 `WKWebView` 验证本机多记录迁移、历史切换、逐条删除、默认档案预填和效果图历史鉴权加载。

## 发布顺序

1. 发布后端/H5，保持 `ANJU_ENABLE_KNOWLEDGE_ADVISOR=0`、`ANJU_ENABLE_RTC_VIDEO_ADVISOR=0` 与 `ANJU_ENABLE_IOS_HOME_CAMERA=0`。
2. 完成通用顾问匿名恢复、文字问答、字幕幂等、RTC 排队与隐私删除冒烟后，再开启 `ANJU_ENABLE_KNOWLEDGE_ADVISOR=1`。
3. 在公网候选环境完成回调与模型能力门禁，再发布新 iOS 并完成双设备冒烟。
4. 先开启 `ios_home_camera`，再小范围开启 `rtc_video_advisor`；持续观察回退率、时延、超时、上传失败和容器重启。
