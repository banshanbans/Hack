...
> **2026-08-07 范围变更：**本文中的游园会、四 Zone、fair API、Turbo/Pro 原生报告任务全部作废，不得继续实施或作为演示口径。当前路线是 iOS `WKWebView` 统一 H5 P01—P09，原生仅负责家庭房间拍照与实时扫描；现行计划见 `docs/CAMERA_UPGRADE_ROADMAP.md`。
...
- **2026-08-06：本地视频关键帧能力取消。** 不接通本地视频选择、自动抽帧和 `video_frame` 正式识别；现有代码/DTO 可兼容保留，但不进入产品、演示、发布门禁和效果宣传。H5 实时相机与 iPhone AR 不受影响。
...
| 对应页面 | P06、P07、P09；照片/H5 相机/iOS 共用 |
| 影响组件 | `assessment_service.py`、`scoring.py`、风险 DTO/测试 |
| 依赖 | 现有 media metadata、规则库 |
| 预计工时 | 1.5—2.5 人日 |
| 测试重点 | region null/非法；同风险两照片；H5/iOS 连续帧相邻/视角变化；不确定合并；用户反馈后重算 |
...
### P0-C — 本地视频关键帧能力取消，不再实施

| 字段 | 内容 |
|---|---|
| 用户价值 | 缩小范围，避免评委把实时相机、本地视频抽帧和正式照片分析混为一谈 |
| 对应页面 | P01、P04、底部中央相机 |
| 影响组件 | 文档、集中式文案、能力开关；现有代码不要求删除 |
| 依赖 | 无 |
| 预计工时 | 文档/口径收口 0.25 人日；不安排功能开发 |
| 测试重点 | 无本地视频文件选择器；不创建 `video_frame` 正式会话；相机入口仍明确为临时建议 |
| 验收标准 | 产品、比赛材料和演示脚本都不宣称支持视频抽帧识别；遗留代码默认关闭且不可达 |
| 风险 | 删除遗留代码可能引起无关回归，因此本轮允许保留 |
| 是否直接提升体验 | 是，降低入口歧义和现场范围风险 |
...
- 收口本地视频取消口径，确认 `video_frame` 无用户入口；实时相机文案保持独立。
- 报告仅列已选清单并补预算总计。
- 为 3 图/2 用户设置容量等待预算；质量失败与图片不合格分离，并支持同一 media 重试。

### Day 3 — 真机与发布演练

- Safari/Chrome、HEIC/旋转照片、H5 相机权限、弱网/刷新。
